...
function calculatePricing({
  ...
  const baseHourlyRate = baseDailyRate / 24;

  // Calculate rental cost with tier adjustment
  // priceAdjustment is positive for surge, negative for discount
  // For surge: multiplier = 1 + priceAdjustment (e.g., +100% = 2.0x)
  // For discount: multiplier = 1 + priceAdjustment (e.g., -5% = 0.95x)
  const rentalMultiplier = 1 + tier.priceAdjustment;
  const hourlyRate = baseHourlyRate * rentalMultiplier;
  const rentalCost = hours * hourlyRate;

  // Calculate insurance cost (fixed per tier based on divisor)
  const insuranceCost = baseDailyRate / tier.insuranceDivisor;
  ...
  // Calculate GST (18% on rental + insurance + convenience fee)
  ...
  // Apply discount
  const discountAmount = Math.max(0, Math.min(discount, totalBeforeDiscount));
